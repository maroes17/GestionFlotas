...
function verificarPatenteFlota(patente) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (
      data[i][patenteColumnIndex] &&
      data[i][patenteColumnIndex].toUpperCase() === patente.toUpperCase()
    ) {
...
function obtenerRegistroFlota(idFlota) {
    try {
      if (!idFlota) throw new Error("ID de flota no proporcionado");
      
      const ss = SpreadsheetApp.openById("1RqztNinU7VGagy8P2wrTLqtXsXLr1W-Dlo_72c5OxiY");
      const sheet = ss.getSheetByName("Flota");
      const data = sheet.getDataRange().getValues();
      const idColumnIndex = 0;
      const idBuscado = String(idFlota).trim();
  
      for (let i = 1; i < data.length; i++) {
        const idActual = String(data[i][idColumnIndex]).trim();
        
        if (idActual === idBuscado) {
          // Convertir objetos Date a strings ISO para consistencia
          const registro = data[i].map(cell => {
      ...
      console.error("Error en obtenerRegistroFlota:", error);
      throw error;
    }
  }
...
function actualizarRegistroFlota(registroModificado) {
    try {
      ...
      const sheet = ss.getSheetByName("Flota");
      const data = sheet.getDataRange().getValues();
      const idColumnIndex = 0; // Columna del ID
      
      // Buscar la fila correspondiente al ID
      for (let i = 1; i < data.length; i++) {
        if (String(data[i][idColumnIndex]).trim() === String(registroModificado[0]).trim()) {
          // Actualizar cada celda de la fila excepto la fecha de creación
          for (let j = 0; j < registroModificado.length; j++) {
            if (j !== 11) { // Excluir la columna de fecha de creación (ajustar índice según tu estructura)
